...
# `features/auth/demo` seit Phase 21.5 und `features/auth/zustimmung` seit 21.2:
# `konten.ts` importiert beide, also werden sie mitgeladen — und eine Liste von
# Dateinamen in einem Skript wird irgendwann nicht nachgezogen (FALLEN.md). Sie
# wird hier nachgezogen. **Zweimal hintereinander jetzt**, bei zwei aufeinander
# folgenden Phasen: Diese Liste ist die Sorte Stelle, die man beim nächsten Mal
# wieder vergisst, und ihr eigener Kommentar ist die einzige Erinnerung daran.
for DATEI in features/auth/anmeldung features/auth/demo features/auth/konto features/auth/konten features/auth/zustimmung lib/supabase lib/sitzungsspeicher; do
...
echo "✓ anmeldung.js, demo.js, konto.js, konten.js, zustimmung.js, supabase.js und sitzungsspeicher.js laufen in blankem Node."
