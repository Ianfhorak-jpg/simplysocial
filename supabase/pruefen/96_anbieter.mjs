...
const attrappe = await import(`${ARBEIT}/attrappe/anmeldung.mjs`);
const supabase = await import(`${ARBEIT}/supabase/anmeldung.mjs`);
...
pruef('Apple steht VOR Google', attrappe.ANMELDE_WEGE.indexOf('apple') < attrappe.ANMELDE_WEGE.indexOf('google'), true);
pruef('drei Wege, nicht mehr und nicht weniger', [...attrappe.ANMELDE_WEGE], ['apple', 'google', 'email-code']);
// Beide Fassungen werden vom Prüfstand ERZWUNGEN (siehe 96_anbieter.sh), keine
// stammt aus dem Repo. Diese zwei Zeilen belegen, dass die Erzwingung durch die
// Übersetzung bis hierher durchgekommen ist — nicht, wie der Schalter gerade
// steht. Das ist seit dem 2026-09-13 der Unterschied: Der Repo-Stand ist Ians
// Entscheidung, keine Regel, und ein Prüfstand, der ihn vorschreibt, wird nach
// dem ersten Umlegen abgeschaltet statt gelesen.
pruef('die erste Fassung ist attrappe', attrappe.ANMELDE_QUELLE, 'attrappe');
pruef('die zweite Fassung ist supabase', supabase.ANMELDE_QUELLE, 'supabase');
...
// nicht supabase ist, ändert 20.3-b2 am Prototyp NICHTS.
for (const lage of [WEB, GERAET]) {
  const name = lage.apple ? 'Gerät' : 'Web';
  for (const weg of attrappe.ANMELDE_WEGE) {
    const { bereit, hinweis } = attrappe.anmeldeFolgen(weg, lage);
...
}
{
  ...
  const saetze = [...new Set(attrappe.ANMELDE_WEGE.map((w) => attrappe.anmeldeFolgen(w, WEB).hinweis))].filter(Boolean);
  pruef('der Bildschirm zeigt genau EINEN Hinweis', saetze.length, 1);
}

console.log('\n── NACH dem Umlegen: das Gerät kann, der Browser nicht ──');
pruef('Gerät/apple ist bereit', supabase.anmeldeFolgen('apple', GERAET).bereit, true);
pruef('Gerät/google ist bereit', supabase.anmeldeFolgen('google', GERAET).bereit, true);
pruef('Gerät/email-code ist bereit', supabase.anmeldeFolgen('email-code', GERAET).bereit, true);
...
pruef('Web/apple ist NICHT bereit', supabase.anmeldeFolgen('apple', WEB).bereit, false);
pruef('Web/google ist NICHT bereit', supabase.anmeldeFolgen('google', WEB).bereit, false);
pruef('Web/email-code ist trotzdem bereit', supabase.anmeldeFolgen('email-code', WEB).bereit, true);
pruef('Gerät ohne .env-Eintrag: Google zu, Apple offen', [
  supabase.anmeldeFolgen('google', OHNE_GOOGLE).bereit,
  supabase.anmeldeFolgen('apple', OHNE_GOOGLE).bereit,
], [false, true]);
...
for (const weg of ['apple', 'google']) {
  pruef(`Web/${weg}: sagt, dass es nur am Handy geht`, supabase.anmeldeFolgen(weg, WEB).hinweis, 'Geht nur in der App am Handy.');
  pruef(`Gerät/${weg}: sagt GAR NICHTS mehr`, supabase.anmeldeFolgen(weg, GERAET).hinweis, '');
}
pruef('Gerät/email-code erklärt den Code', supabase.anmeldeFolgen('email-code', GERAET).hinweis, 'Wir schicken dir eine Zahl, kein Passwort.');
...
pruef('abgebrochen → null, also steht nichts da', attrappe.anbieterFehlerText('abgebrochen'), null);
for (const code of ['nicht-hier', 'kein-zugang', 'kein-ausweis', 'unbekannt', 'irgendwas']) {
  const text = attrappe.anbieterFehlerText(code);
...
pruef(
  ...
  attrappe.googleRueckweg('132215869376-abc123.apps.googleusercontent.com'),
  'com.googleusercontent.apps.132215869376-abc123:/oauthredirect',
);
pruef(
  'der Suffix wird nur am ENDE abgeschnitten',
  attrappe.googleRueckweg('x.apps.googleusercontent.com.apps.googleusercontent.com'),
...
pruef(
  ...
  attrappe.googleRueckweg('132215869376-abc123'),
  'com.googleusercontent.apps.132215869376-abc123:/oauthredirect',
);

console.log('\n── Der Torwächter hat vier Glieder, und keines ist dazugekommen ──');
pruef('unbekannt → wartet', attrappe.torwaechterZeigt({ zustand: 'unbekannt' }), 'wartet');
pruef('aus → anmelden', attrappe.torwaechterZeigt({ zustand: 'aus' }), 'anmelden');
...
pruef('neu MIT Namen → weiter erstes-konto', attrappe.torwaechterZeigt({ zustand: 'neu', authId: 'u', email: 'a@b.at', name: 'Ian' }), 'erstes-konto');
pruef('neu OHNE Namen → dasselbe', attrappe.torwaechterZeigt({ zustand: 'neu', authId: 'u', email: 'a@b.at' }), 'erstes-konto');
pruef('an → app', attrappe.torwaechterZeigt({ zustand: 'an', ichId: 'u' }), 'app');
